...
const INITIAL_VERSION_LABEL = "未上架初始版本";
...
const defaults = {
  ...
  smile: { label: "進來\n笑一下", href: "smile.html", image: "assets/populove-bear-icon.svg", giphyKey: "", enabled: true },
  floatButtons: [{ id: "fb1", text: "奇幻角色生成器", href: "https://character-prompt-generator.netlify.app/", color: "#ff8a1f", hidden: false, enabled: true }, { id: "fb2", text: "翊軒酒莊", href: "https://godenwine.com/", color: "#7a5cff", hidden: false, enabled: true }],
  customMemes: [],
...
function normalizeVersionHistory(items){const list=Array.isArray(items)?items:[]; const initial=list.find((item)=>item.id===INITIAL_VERSION_ID); const rest=list.filter((item)=>!isProtectedVersion(item)); const initialVer=protectedVersion(INITIAL_VERSION_ID,INITIAL_VERSION_LABEL,initial,initialUnsetProductPayload); const loginTime=Number(localStorage.getItem("populoveAdminLastLogin")||0); if(loginTime)initialVer.createdAt=loginTime; return [initialVer,...rest].slice(0,versionLimit());}
...
function deleteVersion(versionId){const history=readVersionHistory(); const item=history.find((entry)=>entry.id===versionId); if(isProtectedVersion(item)){status("#versionStatus","內建版本無法刪除。"); renderVersions(); return;} if(!window.confirm("確定要刪除這個版本嗎？")){renderVersions(); return;} const next=history.filter((entry)=>entry.id!==versionId); saveVersionHistory(next); renderVersions(); status("#versionStatus","已刪除版本。");}
...
function bindVersionSwipeHint(){const head=$("#versionHistoryList")?.closest(".admin-list-wrap")?.querySelector(".admin-list-head"); head?.querySelector(".version-swipe-hint")?.remove();}
...
function normalizeSmileEntryAdmin(data){
  ...
  return {...data, label, image, enabled};
}
function normalizeFloatButtons(items){return (Array.isArray(items)?items:[]).slice(0,2).map((item,index)=>{const data=item||{}; const rawColor=String(data.color||"").trim(); const color=/^#?[0-9a-fA-F]{3,8}$/.test(rawColor)?(rawColor.startsWith("#")?rawColor:`#${rawColor}`):"#ffb12a"; return {id:data.id||id("fb"),text:String(data.text||"").trim(),href:String(data.href||"").trim(),color,hidden:!!data.hidden,enabled:data.enabled!==false};});}
function floatButtonCard(button,index){return `<div class="admin-floatbtn" data-float-item="${escapeHtml(button.id)}"><div class="admin-floatbtn-head"><strong>懸浮按鈕 ${index+1}</strong><button class="admin-floatbtn-remove" type="button" data-remove-floatbtn="${escapeHtml(button.id)}">移除</button></div><label>連結資訊（顯示文字）<input type="text" data-fb="text" value="${escapeHtml(button.text)}" placeholder="例如：奇幻角色生成器"></label><label>超連結<input type="text" data-fb="href" value="${escapeHtml(button.href)}" placeholder="https://"></label><label class="admin-color-field">顏色<input type="color" data-fb="color" value="${escapeHtml(button.color||"#ffb12a")}"></label><div class="admin-floatbtn-toggles"><label class="admin-toggle"><input type="checkbox" data-fb="enabled"${button.enabled!==false?" checked":""}><span class="toggle-track" aria-hidden="true"></span><span>是否作用（可點擊）</span></label><label class="admin-toggle"><input type="checkbox" data-fb="hidden"${button.hidden?" checked":""}><span class="toggle-track" aria-hidden="true"></span><span>是否隱藏</span></label></div></div>`;}
function syncFloatButtonsFromDom(){const list=$("#floatBtnList"); if(!list)return; floatButtonsDraft=[...list.querySelectorAll("[data-float-item]")].map((node)=>({id:node.dataset.floatItem||id("fb"),text:node.querySelector('[data-fb="text"]')?.value.trim()||"",href:node.querySelector('[data-fb="href"]')?.value.trim()||"",color:node.querySelector('[data-fb="color"]')?.value||"#ffb12a",enabled:!!node.querySelector('[data-fb="enabled"]')?.checked,hidden:!!node.querySelector('[data-fb="hidden"]')?.checked}));}
function renderFloatButtonsList(){const list=$("#floatBtnList"); if(!list)return; list.innerHTML=floatButtonsDraft.length?floatButtonsDraft.map((button,index)=>floatButtonCard(button,index)).join(""):`<p class="admin-floatbtn-empty admin-note">尚未新增懸浮按鈕，點下方「新增懸浮按鈕」加入（最多 2 個）。</p>`; const add=$("#addFloatButton"); if(add)add.disabled=floatButtonsDraft.length>=2;}
...
const clampSize=(value,min,max)=>{const n=parseInt(value,10); return Number.isFinite(n)?Math.min(max,Math.max(min,n)):"";};
$("#bannerForm").addEventListener("submit",(e)=>{e.preventDefault(); const f=e.currentTarget; upsert(STORAGE.banners,defaults.banners,{id:f.elements.id.value||id("b"),label:f.elements.label.value.trim(),title:f.elements.title.value.trim(),title2:f.elements.title2.value.trim(),text:f.elements.text.value.trim(),titleSize:clampSize(f.elements.titleSize.value,24,80),textSize:clampSize(f.elements.textSize.value,14,28),image:bannerImage}); f.reset(); bannerImage="assets/hero-teamwear.png"; $("#bannerPreview img").src=bannerImage; status("#bannerStatus","Banner 已儲存。");});
...
$("#smileEntryForm").addEventListener("submit",(e)=>{e.preventDefault(); const f=e.currentTarget; const initialCoins=Math.max(0,Math.floor(Number(f.elements.initialCoins?.value||defaults.coinSettings.initialCoins)||0)); syncFloatButtonsFromDom(); const floatButtons=normalizeFloatButtons(floatButtonsDraft); const reactionSettings={visible:{"Populove!!!":true}}; CONFIGURABLE_REACTIONS.forEach((reaction,index)=>{reactionSettings.visible[reaction]=!!f.elements[`reactionEnabled${index+1}`]?.checked;}); save(STORAGE.smile,{label:f.elements.label.value.trim(),href:f.elements.href.value.trim(),image:smileEntryImage,giphyKey:f.elements.giphyKey?.value.trim()||"",enabled:!!f.elements.smileEnabled?.checked}); save(STORAGE.coinSettings,{initialCoins}); save(STORAGE.floatButtons,floatButtons); save(STORAGE.reactionSettings,reactionSettings); save(STORAGE.smileDisplay,{showHotTag:!!f.elements.showHotTag?.checked,showPopuloveReaction:!!f.elements.showPopuloveReaction?.checked,showCoinBadge:!!f.elements.showCoinBadge?.checked}); localStorage.removeItem("populoveGiphyMemeCache"); floatButtonsDraft=floatButtons; renderFloatButtonsList(); status("#smileEntryStatus","快捷按鈕設定已儲存（笑一下入口、P coin、懸浮按鈕、類別與反應顯示）。");});
$("#addFloatButton")?.addEventListener("click",()=>{syncFloatButtonsFromDom(); if(floatButtonsDraft.length>=2){status("#smileEntryStatus","懸浮按鈕最多 2 個。"); return;} floatButtonsDraft.push({id:id("fb"),text:"",href:"",color:"#ffb12a",hidden:false,enabled:true}); renderFloatButtonsList();});
$("#floatBtnList")?.addEventListener("click",(event)=>{const remove=event.target.closest("[data-remove-floatbtn]"); if(!remove)return; syncFloatButtonsFromDom(); floatButtonsDraft=floatButtonsDraft.filter((button)=>button.id!==remove.dataset.removeFloatbtn); renderFloatButtonsList();});
...
bindList("#bannerList",STORAGE.banners,defaults.banners,"#bannerForm","#bannerStatus","banner",(b)=>{const f=$("#bannerForm"); f.elements.id.value=b.id; f.elements.label.value=b.label; f.elements.title.value=b.title; f.elements.title2.value=b.title2||""; f.elements.text.value=b.text; f.elements.titleSize.value=b.titleSize||""; f.elements.textSize.value=b.textSize||""; bannerImage=b.image; $("#bannerPreview img").src=b.image;},()=>{});
...
function bindRowActions(listSelector,key,fallback,type,statusSelector){
  const list=$(listSelector); if(!list)return;
  list.addEventListener("click",(event)=>{const mv=event.target.closest("[data-move]"); if(!mv)return; const items=read(key,fallback); const i=items.findIndex((x)=>x.id===mv.dataset.moveId); if(i<0)return; const j=mv.dataset.move==="up"?i-1:i+1; if(j<0||j>=items.length)return; const tmp=items[i]; items[i]=items[j]; items[j]=tmp; save(key,items); renderAll(); status(statusSelector,"順序已更新。");});
  list.addEventListener("touchstart",(event)=>{const r=event.target.closest(".admin-product"); if(!r)return; r.dataset.sx=String(event.changedTouches[0].clientX); r.dataset.sy=String(event.changedTouches[0].clientY);},{passive:true});
  list.addEventListener("touchend",(event)=>{const r=event.target.closest(".admin-product"); if(!r||r.dataset.sx===undefined)return; const dx=event.changedTouches[0].clientX-Number(r.dataset.sx); const dy=event.changedTouches[0].clientY-Number(r.dataset.sy); delete r.dataset.sx; delete r.dataset.sy; if(dx<-56&&Math.abs(dx)>Math.abs(dy)*1.5){const btn=r.querySelector(`[data-remove-${type}]`); if(!btn)return; const rid=btn.getAttribute(`data-remove-${type}`); if(!window.confirm("確定刪除這一筆？"))return; const items=read(key,fallback).filter((x)=>x.id!==rid); save(key,items); renderAll(); status(statusSelector,"已刪除。");}},{passive:true});
}
bindRowActions("#productList",STORAGE.products,defaults.products,"product","#productStatus");
bindRowActions("#categoryList",STORAGE.categories,defaults.categories,"category","#categoryStatus");
bindRowActions("#bannerList",STORAGE.banners,defaults.banners,"banner","#bannerStatus");
bindRowActions("#cardList",STORAGE.contactCards,defaults.contactCards,"card","#cardStatus");
bindRowActions("#flowList",STORAGE.flow,defaults.flow,"flow","#flowStatus");
